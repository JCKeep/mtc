    ...
    <div class="forms-container">
      <div class="signin-signup">
        ...
        <el-form
            ...
            v-show="loginOrForget == true"
        >
          <p style="font-size: 24px;color: rgba(85, 166, 132, 1);margin-left: 30px;margin-top: -10px">登录页面</p>
          <el-form-item label="邮箱" prop="email" label-width="25%" style="margin-top: 15px">
          ...
          <el-form-item label-width="36px">
            <div class="tiparea flex justify-space-between mb-4 flex-wrap gap-4">
              <p style="font-size: 20px">忘记密码？
                <el-button style="font-size: 20px;margin-top: -3px" type="success" @click="loginOrForget = false" link>
                  立即找回
                </el-button>
              </p>
            </div>
        ...
        <el-form
            ...
            size="large"
            class="loginForm sign-in-form"
            v-show="loginOrForget == false"
        >
          <p style="font-size: 24px;color: rgba(85, 166, 132, 1);margin-left: 30px;margin-top: -10px">忘记密码页面</p>
          <el-form-item label="邮箱账号" prop="email" label-width="30%" style="margin-top: 15px">
          ...
          <el-form-item label="验证码" prop="identify" label-width="30%">
            <div style="width: 148px">
              <el-input
                  v-model="forgetForm.identify"
                  type="text"
                  placeholder="请输入验证码"
              />
            </div>
            <el-button style="margin-left: 10px" @click="sendCode2">发送验证码</el-button>
          </el-form-item>

          <el-form-item label="重置密码" prop="passNew" label-width="30%">
            <el-input v-model="forgetForm.passNew" type="password" placeholder="请输入密码"/>
          </el-form-item>
          <el-form-item label="确认密码" prop="checkPass" label-width="30%">
            <el-input
          ...
          <el-form-item>
            <div class="tiparea flex justify-space-between mb-4 flex-wrap gap-4">
              <el-button style="margin-left: -90px;font-size: 20px;margin-top: -3px" type="success"
                         @click="loginOrForget = true" link>返回登录
              </el-button>
            </div>
            <el-button @click="submit()" style="margin-left: 33px;width: 150px" class="submit-btn">确认
            </el-button>
        ...
        <el-form
          ...
          <p style="font-size: 24px;color: rgba(85, 166, 132, 1);margin-left: 30px;margin-top: -10px">注册页面</p>
          <el-form-item label="用户名" prop="name" label-width="30%" style="margin-top: 15px">
          ...
          <el-form-item>
            <el-button @click="handleRegister" style="margin-left: 135px;width: 327px" class="submit-btn">注册
            </el-button>
            <!--      <el-button @click="handleRegister('registerForm')" class="submit-btn">注册</el-button>-->
          </el-form-item>
...
const loginOrForget = ref(true)
const signUpMode = ref<boolean>(false);

const validatePass = (rule: any, value: any, callback: any) => {
  //必须包含⼤⼩写字母、数字、特殊字符长度6-19位之间
...
const validatePassForget = (rule: any, value: any, callback: any) => {
  if (value !== forgetForm.passNew) {
    callback(new Error("两次输入的密码不一致"))
  } else {
    callback()
  }
...
const forgetRules = reactive({
  email: [{required: true, message: '请输入邮箱', trigger: 'blur'},
    {type: 'email', message: '邮箱格式有误，请重新输入', trigger: 'blur'},
    {
      ...
      message: "请输入正确的邮箱格式，检测到非法字符",
      trigger: 'blur'
    }],
  identify: [{required: true, message: "请输入验证码", trigger: 'blur'},
    {validator: checkCodeForget, trigger: 'blur'}],
  passNew: [{required: true, message: "请输入密码", trigger: 'blur'},
    {validator: validatePass, trigger: 'blur'}],
  checkPass: [{required: true, message: "请再次输入密码", trigger: 'blur'},
    {validator: validatePass, trigger: 'blur'},
    {validator: validatePassForget, trigger: 'blur'}],
})

function checkCodeForget(rule: any, value: any, callback: any) {
  if (forgetForm.identify != forgetForm.code) {
...
function sendCode() {
  axios.get("/mtc/user/getcode",
      {params:{email:forgetForm.email}}).then((res:any)=>{
    forgetForm.code = res.data;
...
function sendCode2() {
  axios.get("/mtc/user/getcode2",
      {params:{email:forgetForm.email}}).then((res:any)=>{
        if (res.data == 'FUCK YOU'){
          ElMessage.error("当前账号未注册")
        }else {
          forgetForm.code = res.data;
          ElMessage.success("验证码发送成功")
        }
...
function submit() {
  if (forgetForm.checkPass != forgetForm.passNew){
    ElMessage.error("两次输入的密码不一致！")
  }else {
    axios.post("/mtc/user/update", {
      userEmail:forgetForm.email,
      code:forgetForm.identify,
      userPasswordNew:forgetForm.checkPass,
    }).then((Response)=>{
      if(Response.data.code===200){
        ElMessage.success("密码修改成功")
        loginOrForget.value = true;
      }else{
